...
## Summary
A polling-based sensor grid system consisting of four ESP32-S3 devices communicating wirelessly via ESP-NOW. Unlike sensorgrid_v1 where sensors broadcast freely, in v2 the server controls all communication: it discovers sensors, registers them, and then polls each sensor in round-robin order for data. This eliminates collision risk when using many sensors or large data packets.

## System Object Model

![sensorgrid_v2 object model](img/sensorgrid_v2_object_model.svg)

### App List

| App | Device(s) | Responsibility |
|-----|-----------|---------------|
| **sensor_v2** | ACM1, ACM2 | Reactive: responds to DISCOVER with REGISTER, responds to POLL with DATA. Each instance has a unique sensor ID and independently advances a simulated value. |
| **server_v2** | ACM0 | Runs a WiFi access point, discovers and registers sensors via broadcast, polls them in round-robin order via unicast, and serves a web dashboard with real-time bar charts and a JSON API. Flashes LED when sensors are missing. |
| **client_v2** | ACM3 | Connects to the server's WiFi AP and runs automated HTTP tests against all web endpoints, reporting PASS/FAIL results via serial log. |

### Communication Protocol

#### Phase 1: Discovery
- **server_v2 -> all sensors**: ESP-NOW broadcast of `DiscoverPacket` every 500ms.
- **sensor_v2 -> server_v2**: ESP-NOW unicast of `RegisterPacket` (sensor ID) in response to DISCOVER.
- Server collects registrations until all expected sensors have registered, then transitions to polling.

#### Phase 2: Polling
- **server_v2 -> sensor_v2**: ESP-NOW unicast of `PollPacket` (target sensor ID) to each registered sensor in round-robin order.
- **sensor_v2 -> server_v2**: ESP-NOW unicast of `DataPacket` (sensor ID + payload) in response to POLL.
...
#### Web Interface
- **client_v2 -> server_v2**: WiFi STA connection to the server's AP, followed by HTTP GET requests to `/` (dashboard) and `/api/sensors` (JSON).
- **server_v2 -> client_v2**: HTTP responses containing HTML (dashboard page) or JSON (sensor data).
...
//#include <client_v2.ino>
```
In `apps/sensorgrid_v2/sensor_v2/src/sensor_v2_ino.h`, set the sensor ID:
```cpp
...
Change the sensor ID in `sensor_v2_ino.h`:
```cpp
