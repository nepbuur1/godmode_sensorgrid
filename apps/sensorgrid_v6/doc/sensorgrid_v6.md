...
#### Web Interface
- **client_v6 -> server_v6**: WiFi STA connection to the server's AP, followed by HTTP GET requests to `/` (dashboard), `/grid` (grid visualization), `/api/sensors` (JSON summary), `/api/measurements/{1..4}` (JSON measurement arrays per sensor), and `/api/allmeasurements` (all sensors' measurements in one response).
- **server_v6 -> client_v6**: HTTP responses containing HTML (dashboard or grid page) or JSON (sensor data).
...
|------|-----------|------|
| /dev/ttyACM3 | Espressif USB JTAG (Dev 006) | server_v6 |
| /dev/ttyACM2 | Espressif USB JTAG (Dev 005) | client_v6 |
| /dev/ttyACM1 | Espressif USB JTAG (Dev 004) | sensor_v6 (ID=1) |
| /dev/ttyACM0 | Espressif USB JTAG (Dev 003) | sensor_v6 (ID=2) |
| /dev/ttyUSB0 | CP210x UART Bridge (Dev 002) | sensor_v6 (ID=3) |
...
```cpp
#include <server_v6.ino>
//#include <sensor_v6.ino>
//#include <client_v6.ino>
```
...
//#include <client_v6.ino>
```
In `apps/sensorgrid_v6/sensor_v6/src/sensor_v6_ino.h`, set the sensor ID:
```cpp
...
Change the sensor ID in `sensor_v6_ino.h`:
```cpp
