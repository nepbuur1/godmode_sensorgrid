...
namespace crt
{
	...
	private:
		...
		static const unsigned long DISCOVER_WINDOW_MS = 10000;
		...
		void updateLed()
		{
			if (currentState == State::DISCOVERING)
			{
				...
				if (now - lastLedToggleMs >= LED_FLASH_INTERVAL_MS)
				{
					lastLedToggleMs = now;
					ledOn = !ledOn;
#ifdef USE_TIMED_DISCOVERY
					bool withinWindow = (now - initMs < DISCOVER_WINDOW_MS);
					if (withinWindow)
					{
						// Green flash during discovery window
						neopixelWrite(RGB_BUILTIN, 0, ledOn ? 20 : 0, 0);
					}
					else
					{
						// Red flash after window expired with no sensors
						neopixelWrite(RGB_BUILTIN, ledOn ? 20 : 0, 0, 0);
					}
#else
					// Original v5 behaviour: red flash while waiting for sensors
					neopixelWrite(RGB_BUILTIN, ledOn ? 20 : 0, 0, 0);
#endif
				}
			}
...
#ifdef USE_TIMED_DISCOVERY
			bool windowExpired = (now - initMs >= DISCOVER_WINDOW_MS);
			if (windowExpired && registeredCount >= 1)
			{
				ESP_LOGI("ServerNode",
						 "Discovery window elapsed, %u sensor(s) registered, starting POLL cycle",
						 registeredCount);
				currentPollIndex = 0;
				currentState = State::POLLING;
			}
			// If all expected sensors arrived early, no need to wait
			if (registeredCount >= expectedSensorCount)
			{
				ESP_LOGI("ServerNode",
						 "All %u sensors registered early, starting POLL cycle",
						 expectedSensorCount);
				currentPollIndex = 0;
				currentState = State::POLLING;
			}
	...
	public:
		ServerNode(const char* ssid, const char* pass, int channel,
			...
			: apSsid(ssid), apPass(pass), apChannel(channel),
			  ...
			  lastLedToggleMs(0), ledOn(false), initMs(0), registeredCount(0)
		{
		}

		void init()
		{
			ESP_LOGI("ServerNode", "Server node v6 starting...");
