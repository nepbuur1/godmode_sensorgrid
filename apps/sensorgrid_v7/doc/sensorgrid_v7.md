...
#### Web Interface
- **client_v7 -> server_v7**: WiFi STA connection to the server's AP, followed by HTTP GET requests to `/` (dashboard), `/grid` (grid visualization), `/api/sensors` (JSON summary), `/api/measurements/{1..4}` (JSON measurement arrays per sensor), and `/api/allmeasurements` (all sensors' measurements in one response).
- **server_v7 -> client_v7**: HTTP responses containing HTML (dashboard or grid page) or JSON (sensor data).
...
|------|-----------|------|
| /dev/ttyACM3 | Espressif USB JTAG (Dev 006) | server_v7 |
| /dev/ttyACM2 | Espressif USB JTAG (Dev 005) | client_v7 |
| /dev/ttyACM1 | Espressif USB JTAG (Dev 004) | sensor_v7 (ID=1) |
| /dev/ttyACM0 | Espressif USB JTAG (Dev 003) | sensor_v7 (ID=2) |
| /dev/ttyUSB0 | CP210x UART Bridge (Dev 002) | sensor_v7 (ID=3) |
...
//#include <client_v7.ino>
```
In `apps/sensorgrid_v7/sensor_v7/src/sensor_v7_ino.h`, set the sensor ID:
```cpp
...
Change the sensor ID in `sensor_v7_ino.h`:
```cpp
