    ...
    while(file){
        if(file.isDirectory()){
            ...
            if(levels){
                ...
                // mave: temp buffer needed to prevent overlap error.
                char temp_buffer[sizeof(buffer)]; // tijdelijke buffer
                snprintf(temp_buffer, sizeof(temp_buffer), "%s%s", buffer, file.name());
                snprintf(buffer, sizeof(buffer), "%s", temp_buffer);
