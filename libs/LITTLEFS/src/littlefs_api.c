...
int littlefs_api_read(const struct lfs_config *c, lfs_block_t block,
    ...
    if (err) {
        ESP_LOGE(TAG, "failed to read addr %08u, size %08lx, err %d", part_off, size, err);
        return LFS_ERR_IO;
    }
    return 0;
}
...
int littlefs_api_prog(const struct lfs_config *c, lfs_block_t block,
    ...
    if (err) {
        ESP_LOGE(TAG, "failed to write addr %08u, size %08lx, err %d", part_off, size, err);
        return LFS_ERR_IO;
    }
    return 0;
}
...
int littlefs_api_erase(const struct lfs_config *c, lfs_block_t block) {
    ...
    if (err) {
        ESP_LOGE(TAG, "failed to erase addr %08u, size %lu, err %d", part_off, c->block_size, err);
        return LFS_ERR_IO;
    }
    return 0;
