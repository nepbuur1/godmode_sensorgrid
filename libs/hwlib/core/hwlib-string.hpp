...
const char * asciz_beyond( const char * p ){
   while( *p != '\0' ){
      ++p;
   }
   return p;
...
private:   
   ...
   template< typename T > 
   string_base( size_t allocated_length, char * content, const T & x ):
   ...
   string_base( char * start, char * beyond ):
...
public:
   ...
   bool valid_index( const size_t n ) const {
   ...
   static const string_base range( 
      const char * start, 
      const char * beyond 
   ){
