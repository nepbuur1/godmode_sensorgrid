...
namespace hwlib
{
   ...
   public:
      ...
      ssd1306_i2c(i2c_bus &bus, uint_fast8_t address = 0x3C) : bus(bus),
                                                               address(address),
                                                               cursor(255, 255)
      {
   ...
   public:
      ...
      ssd1306_spi_res_dc_cs(spi_bus &bus, pin_out &res, pin_out &dc, pin_out &cs) : bus(bus),
                                                                                    res(res),
                                                                                    dc(dc),
                                                                                    cs(cs),
                                                                                    cursor(255, 255)
   ...
   private:
      ...
      void clear_implementation(color c) override
      {
         ...
         bus.write_begin(address);
         bus.write_block(&ssd1306_data_prefix, 1);
         for (uint_fast16_t x = 0; x < sizeof(buffer); ++x)
         {
            buffer[x] = d;
            bus.write_block(&d, 1);
   ...
   public:
      /// construct by providing the i2c channel
      glcd_oled_i2c_128x64_direct(i2c_bus &bus, uint_fast8_t address = 0x3C) : ssd1306_i2c(bus, address),
   ...
   /// buffered oled window
   class glcd_oled_spi_128x64_direct_res_dc_cs : public ssd1306_spi_res_dc_cs,
                                                 public window
   {
   private:
      static auto constexpr wsize = xy(128, 64);
   ...
   public:
      /// construct by providing the i2c channel
      glcd_oled_spi_128x64_direct_res_dc_cs(spi_bus &bus, pin_out &res, pin_out &dc, pin_out &cs) : ssd1306_spi_res_dc_cs(bus, res, dc, cs),
                                                                                                    window(wsize, white, black)
      {
   ...
   public:
      /// construct by providing the i2c channel
      glcd_oled_i2c_128x64_buffered(i2c_bus &bus, int address = 0x3C) : ssd1306_i2c(bus, address),
                                                                        window(wsize, white, black)
      {
      ...
      void flush() override
      {
         ...
         bus.write_begin(address);
         bus.write_block(&ssd1306_data_prefix, 1);
         bus.write_block(buffer, sizeof(buffer) / sizeof(uint8_t));
         bus.write_end();
   ...
   public:
      /// construct by providing the i2c channel
      glcd_oled_i2c_128x64_fast_buffered(i2c_bus &bus, int address = 0x3C) : ssd1306_i2c(bus, address),
                                                                             window(wsize, white, black)
      {
         bus.write(address, ssd1306_initialization,
                   sizeof(ssd1306_initialization) / sizeof(uint8_t));
      ...
      void flush() override
      {
         ...
         while (start < buf_size)
         {
            ...
            bus.write_begin(address);
            bus.write_block(&ssd1306_data_prefix, 1);
            bus.write_block(&buffer[start], 1 + (tail - start));
            bus.write_end();
         }
      }
